...
use Pyz\Zed\CustomerMerchantPortalGui\Communication\ConfigProvider\CustomerConfigurationProviderInterface;
...
class CustomerGuiTableDataProvider extends AbstractGuiTableDataProvider
{
    ...
    protected function fetchData(AbstractTransfer $criteriaTransfer): GuiTableDataResponseTransfer
    {
        ...
        foreach ($customerCollectionTransfer->getCustomers() as $customerTransfer) {
            $responseData = [
                CustomerConfigurationProviderInterface::COL_KEY_ID_CUSTOMER => $customerTransfer->getIdCustomer(),
                CustomerConfigurationProviderInterface::COL_KEY_CREATED_AT => $customerTransfer->getCreatedAt(),
                CustomerConfigurationProviderInterface::COL_KEY_EMAIL => $customerTransfer->getEmail(),
                CustomerConfigurationProviderInterface::COL_KEY_LAST_NAME => $customerTransfer->getLastName(),
                CustomerConfigurationProviderInterface::COL_KEY_FIRST_NAME => $customerTransfer->getFirstName(),
            ];
            $guiTableRowResponseTransfer = new GuiTableRowDataResponseTransfer();
            $guiTableDataResponseTransfer->addRow(
                $guiTableRowResponseTransfer->setResponseData($responseData)
            );
        }
